...
async def test_apply_actions_reuses_existing_rendered_order(monkeypatch):
    ...
    async def fake_get_rendered_draft(_pool, chat_id):
        assert chat_id == 10
        from src.llm.actions_applier import normalize_render_params
        ...
        return {
            ...
            "order_status": "new",
            "details_json": normalize_render_params(RenderPartitionAction(
                shape="Прямая",
                height=2.5,
                width_a=3,
                partition_type="sliding_2",
...
async def test_apply_actions_suppresses_render_during_measurement_flow(monkeypatch):
    ...
    assert any(c[0] == "state" and c[2] == "scheduling" for c in calls)
...
@pytest.mark.asyncio
async def test_apply_actions_blocks_hallucinated_measurement_during_materials(monkeypatch):
    calls = []
    ...
    actions = ActionsJson(
        ...
        actions={
            ...
            "state_patch": {"mode": "collecting", "step": "dims"}
        },
    )
    ...
    result = await actions_applier.apply_actions(
        ...
        10,
        None,
        {"mode": "collecting", "step": "materials", "collected_params": {}},
        object(),
        object(),
        SimpleNamespace(),
    )
