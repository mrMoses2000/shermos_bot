...
@pytest.mark.asyncio
async def test_webhook_sigterm_stops_server(monkeypatch):
    """Sending SIGTERM to the event loop triggers graceful shutdown of run_webhook.main."""
    import run_webhook

    # Stub out infrastructure so we don't need real Postgres/Redis/SSL
    async def fake_app_context(app):
        app["pg_pool"] = object()
        app["redis"] = object()
        yield
    ...
    monkeypatch.setattr(run_webhook, "_app_context", fake_app_context)
    monkeypatch.setattr(run_webhook.web, "AppRunner", FakeRunner)
    monkeypatch.setattr(run_webhook.web, "TCPSite", FakeSite)
    monkeypatch.setattr(run_webhook, "_ssl_context", lambda: None)

    import os

    main_task = asyncio.create_task(run_webhook.main())
    await asyncio.sleep(0.05)
    os.kill(os.getpid(), signal.SIGTERM)
    await asyncio.wait_for(main_task, timeout=3.0)
    assert main_task.done()
...
@pytest.mark.asyncio
async def test_webhook_shutdown_logs_are_emitted(monkeypatch):
    """webhook_shutdown_initiated and webhook_shutdown_complete are logged.
    ...
    monkeypatch.setattr(run_webhook.logger, "info", fake_info)
    ...
    assert "webhook_shutdown_initiated" in logged, logged
    assert "webhook_shutdown_complete" in logged, logged
