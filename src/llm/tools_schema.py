...
1. render_partition — создать 3D-рендер и расчёт стоимости
   ...
   - handle_sections: список номеров секций для ручек; обязателен, если add_handle=true
   ...
   - Если ручка нужна: "На какой секции поставить ручку?" Для Г- и П-образной также: "На какой стороне будет ручка?"
   ...
   Пример: "на главной 4 секции, по бокам без делений" => П-образная: cols_front=4, cols_left=1, cols_right=1; Г-образная: cols_front=4, cols_side=1.
