...
async def get_manager(pool, phone_e164: str) -> dict[str, Any] | None:
    row = await pool.fetchrow("SELECT * FROM managers WHERE phone_e164=$1", phone_e164)
    return dict(row) if row else None


async def upsert_manager(pool, phone_e164: str, name: str | None = None, is_active: bool = True) -> None:
    await pool.execute(
        """
        INSERT INTO managers (phone_e164, name, is_active)
        VALUES ($1, $2, $3)
        ON CONFLICT (phone_e164) DO UPDATE
        SET name=COALESCE(EXCLUDED.name, managers.name),
...
async def store_otp(pool, phone_e164: str, code_hash: str, expires_at: datetime) -> None:
    await pool.execute(
        """
...
async def get_otp(pool, phone_e164: str) -> dict[str, Any] | None:
    row = await pool.fetchrow("SELECT * FROM auth_otps WHERE phone_e164=$1", phone_e164)
    return dict(row) if row else None


async def increment_otp_attempts(pool, phone_e164: str) -> int:
    return await pool.fetchval(
        "UPDATE auth_otps SET attempts = attempts + 1 WHERE phone_e164=$1 RETURNING attempts",
        phone_e164,
    )


async def delete_otp(pool, phone_e164: str) -> None:
    await pool.execute("DELETE FROM auth_otps WHERE phone_e164=$1", phone_e164)


async def get_otp_rate_limit(pool, phone_e164: str) -> dict[str, Any] | None:
    row = await pool.fetchrow("SELECT * FROM auth_otp_rate_limit WHERE phone_e164=$1", phone_e164)
    return dict(row) if row else None


async def update_otp_rate_limit(pool, phone_e164: str, send_count_1h: int) -> None:
    await pool.execute(
        """
        INSERT INTO auth_otp_rate_limit (phone_e164, last_sent_at, send_count_1h)
        VALUES ($1, now(), $2)
        ON CONFLICT (phone_e164) DO UPDATE
        SET last_sent_at=now(),
            send_count_1h=EXCLUDED.send_count_1h
        """,
        phone_e164,
        send_count_1h,
    )
